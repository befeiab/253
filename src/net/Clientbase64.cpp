...
int64_t Client::send(size_t size)
{
	...
	{
		char base64[4096];
		/*	char *jiami = encode(m_sendBuf);
		memset(m_sendBuf, 0, sizeof(m_sendBuf));
		strcpy(m_sendBuf, jiami);*/
		if(m_sendBuf[daxiao - 1] = '\n') m_sendBuf[daxiao - 1] = '\0';
		//printf("%s", m_sendBuf);
		memset(base64, 0, sizeof(base64));
		base64_encode(m_sendBuf, base64, strlen(m_sendBuf));
		memset(m_sendBuf, 0, sizeof(m_sendBuf));
		strcpy(m_sendBuf, base64);
		strcat(m_sendBuf, "\n");
		daxiao = strlen(m_sendBuf);
	}
	//printf("%s", m_sendBuf);
	//printf("%s", m_sendBuf);
/*	if (m_sendBuf[0] == '{')
	{
		for (int i = 0; m_sendBuf[i]; i++)
			m_sendBuf[i] = m_sendBuf[i]^'?';
	}*/
    ...
    uv_buf_t buf = uv_buf_init(m_sendBuf, (unsigned int) daxiao);
